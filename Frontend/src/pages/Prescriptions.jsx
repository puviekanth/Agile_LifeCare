...
const DeliveryDetails = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`${api}/api/prescriptions/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  // Fixed filtering logic - ensure ALL prescriptions are shown when no verification filter is applied
  const filteredDeliveries = deliveries.filter((d) => {
    ...
    // Verification status filter - FIXED LOGIC
  ...
  return (
    <>
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-10">
        <h2 className="text-3xl font-bold text-center mb-6 text-blue-800 tracking-tight">All Prescriptions</h2>
        ...
        <div className="bg-white p-6 rounded-xl shadow-md mb-8 border border-gray-200">
          <h3 className="text-xl font-semibold text-blue-700 mb-4">Search & Filter Prescriptions</h3>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70">
            <div className="relative bg-white rounded-xl shadow-2xl p-6 max-w-md w-full mx-4">
              ...
              <div className="mb-4">
                ...
                />
                {costError && (
                  <p className="mt-2 text-sm text-red-600">{costError}</p>
                )}
              </div>
